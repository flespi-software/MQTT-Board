 ...
 <div class="absolute-full">
    <q-layout view="hhh LpR lff" container>
      <q-header reveal>
        ...
        <q-btn fab v-if="!activeClient" @click="addClientHandler" icon="mdi-plus" :color="accentColor" style="position: absolute; right: 16px; top: 21px; z-index: 2;"/>
      </q-header>
            ...
            <div v-if="!activeClient" class="absolute scroll" style="top:50px; left: 0; right: 0; bottom: 0;">
              <div v-if="Object.keys(clients).length" class="mqtt-clients row q-pt-md">
                <div class="client__item q-pt-md q-px-md cursor-pointer col-xl-3 col-md-4 col-sm-6 col-xs-12" v-for="(client, index) in clients" :key="index">
                  <q-card :class="{'bg-red-2': !statuses[index], 'bg-green-2': statuses[index]}" @click="setActiveClient(index)">
              ...
              <div v-else class="text-center q-mt-lg text-grey-9 text-weight-bold" style="font-size: 2.5rem;">
                <div>No clients</div>
                <q-btn v-if="!activeClient" @click="addClientHandler">Create client</q-btn>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    setClientStatus (key, status) {
      ...
      this.clients[key].status = status
    },
      ...
      if (clientObj.subscribersStatuses.length && clientObj.subscribersStatuses.filter(status => !!status).length) {
        ...
        if (isResolved) {
          ...
          if (!clientObj.notResolvedFlagInit) {
            clientObj.notResolvedFlagInit = true
    ...
    initClient (key, config) {
      ...
      // mqtt.js v5 handles URL differently - extract host and pass clean options
      const brokerUrl = config.host
      const connectOptions = { ...config }
      delete connectOptions.host // Remove host from options to prevent URL doubling
      ...
      client.on('connect', (connack) => {
        ...
        existedSubscriptionsInSession.forEach(subscription => {
          ...
          subscriber.topic = subscription.topic
          currentStatuses[id] = true
          const grants = [{
            nl: subscription.no_local,
            rap: subscription.rap,
            rh: subscription.retain_handling,
      ...
      client.once('connect', (connack) => {
        clientObj.cid = this.getCid(connack)
        clientObj.inited = true
        this.activateRender()
        let messageBuffer = []
      ...
      client.on('disconnect', (closePacket) => {
        if (closePacket.reasonCode === 143) {
          clientObj.subscribersStatuses.forEach((status, index) => {
            if (status) {
              delete clientObj.client._client._resubscribeTopics[clientObj.subscribers[index].topic]
              clientObj.subscribersStatuses[index] = false
              clientObj.subscribersConnectivityStatuses[index] = false
            }
          })
        }
    ...
    async createClient (settings, index) {
      ...
      this.clients[key].config = config
    ...
    initExternalClients (savedClients) {
      if (savedClients) {
        savedClients.forEach(client => {
          ...
          this.statuses[key] = client.status
          this.clients[key] = currentClient
    ...
    setActiveClient (key) {
      ...
      if (!client) {
        this.showError(new Error(`There is no MQTT client with index ${key}`))
        return false
      }
    ...
    async subscribeMessageHandler (clientKey, subscriberIndex) {
      ...
      if (this.subscribersMessages && this.subscribersMessages.length) {
        this.subscribersMessages[subscriberIndex] = settings.mode === 1 ? {} : []
      }
    ...
    async subscribe (clientKey, subscriberIndex) {
      ...
      try {
        this.clients[clientKey].subscribersStatuses[subscriberIndex] = true
        this.clients[clientKey].subscribersConnectivityStatuses[subscriberIndex] = false
        const grants = await clientObj.client.subscribe(settings.topic, settings.options)
        this.clients[clientKey].subscribersConnectivityStatuses[subscriberIndex] = true
        if (grants.length) {
        ...
        this.clients[clientKey].subscribersStatuses[subscriberIndex] = false
        this.clients[clientKey].subscribersConnectivityStatuses[subscriberIndex] = false
        if (clientObj.status) {
    ...
    playSubscriberHandler (subscriberIndex) {
      this.subscribers[subscriberIndex].missedMessages = undefined
      this.subscribersStatuses[subscriberIndex] = true
    },
    pauseSubscriberHandler (subscriberIndex) {
      this.subscribers[subscriberIndex].missedMessages = 0
      this.subscribersStatuses[subscriberIndex] = 'paused'
    },
    ...
    async unsubscribe (clientKey, subscriberIndex) {
      ...
      try {
        if (!clientObj.subscribersConnectivityStatuses[subscriberIndex]) {
          clientObj.subscribersConnectivityStatuses[subscriberIndex] = true
        }
        clientObj.subscribersStatuses[subscriberIndex] = false
        clientObj.messages[subscriberIndex] = settings.mode === 1 ? {} : []
        clientObj.subscribersMessagesBuffer[subscriberIndex] = []
        ...
        clientObj.subscribersConnectivityStatuses[subscriberIndex] = false
      } catch (e) {
        if (clientObj.status) {
          this.errorHandler(clientKey, e, true)
        } else {
    ...
    makeFlespiRestBus (clientId) {
      ...
      const selectorsAsync = new SelectorsAsync({ flespiApp: `mqtt-board(client: ${client.config && client.config.clientId})` })
      selectorsAsync.bus.auth.getRegions()
        .then((regions) => {
          ...
          if (flespiClientRestRegion) {
            ...
            this.clients[clientId].restBus = selectorsAsync
          } else {
