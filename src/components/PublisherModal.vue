      ...
      <q-card-section :style="{ height: $q.platform.is.mobile || $q.screen.lt.md ? 'calc(100% - 51px)' : '70vh'}" class="scroll q-pa-none">
        <div class="mqtt-client__publisher">
          <div class="q-pa-md">
            ...
            <q-expansion-item :model-value="true" class="bg-grey-2" label="Options">
              <div>
                <div class="q-mx-md" style="line-height: 34px;">
                  ...
                  <q-icon size="24px" color="grey-7" style="margin-left: 12px" name="mdi-information-outline"><q-tooltip>{{getDescription('options.qos')}}</q-tooltip></q-icon>
                </div>
                <div class="q-mr-md q-ml-sm">
                  <q-checkbox style="width: calc(100% - 36px)" color="grey-9" v-model="config.options.retain" label="Retain"/>
                  <q-icon size="24px" color="grey-7" style="margin-left: 12px" name="mdi-information-outline"><q-tooltip max-width="200px">{{getDescription('options.retain')}}</q-tooltip></q-icon>
                </div>
                <div class="q-mr-md q-ml-sm">
                  <q-checkbox style="width: calc(100% - 36px)" color="grey-9" v-model="config.options.dup" label="Duplicate flag"/>
                  <q-icon size="24px" color="grey-7" style="margin-left: 12px" name="mdi-information-outline"><q-tooltip max-width="200px">{{getDescription('options.dup')}}</q-tooltip></q-icon>
                </div>
                <q-expansion-item v-if="version === 5" :model-value="true" class="q-mt-sm q-mb-sm bg-grey-4" label="Properties">
                  <div class="q-px-md q-pb-sm">
                    <div>
                      ...
                      <q-icon size="24px" color="grey-7" style="margin-left: 12px" name="mdi-information-outline"><q-tooltip>{{getDescription('options.properties.payloadFormatIndicator')}}</q-tooltip></q-icon>
                    </div>
                    <q-input
                      ...
                      @update:model-value="(val) => { if (!val) { config.options.properties.messageExpiryInterval = undefined } }"
                    ...
                    >
                      <template #after>
                        <q-icon name="mdi-information-outline"><q-tooltip>{{getDescription('options.properties.messageExpiryInterval')}}</q-tooltip></q-icon>
                      </template>
                    </q-input>
                    <q-input
                      ...
                      @update:model-value="(val) => { if (!val) { config.options.properties.topicAlias = undefined } }"
                      :error="validateSetting('options.properties.topicAlias')"
                      :error-message="getValidateMessage('options.properties.topicAlias')"
                      no-error-icon
                    >
                      <template #after>
                        <q-icon name="mdi-information-outline"><q-tooltip>{{getDescription('options.properties.topicAlias')}}</q-tooltip></q-icon>
                      </template>
                    </q-input>
                    <q-input color="grey-9" v-model="config.options.properties.responseTopic" label="Response topic" outlined class="q-mb-xs" hide-bottom-space>
                      <template #after>
                        <q-icon name="mdi-information-outline"><q-tooltip>{{getDescription('options.properties.responseTopic')}}</q-tooltip></q-icon>
                      </template>
                    </q-input>
                    <q-input color="grey-9" outlined class="q-textarea--fix q-mb-xs" hide-bottom-space type="textarea" autogrow v-model="config.options.properties.correlationData" label="Correlation data">
                      <template #after>
                        <q-icon name="mdi-information-outline"><q-tooltip max-width="200px">{{getDescription('options.properties.correlationData')}}</q-tooltip></q-icon>
                      </template>
                    </q-input>
                    <q-input color="grey-9" outlined class="q-mb-xs" hide-bottom-space v-model="config.options.properties.contentType" label="Content type">
                      <template #after>
                        <q-icon name="mdi-information-outline"><q-tooltip>{{getDescription('options.properties.contentType')}}</q-tooltip></q-icon>
                      </template>
                    </q-input>
                    <div class="q-mb-sm">
                      ...
                      <div>
                        <q-list v-if="config.options.properties.userProperties" class="q-mb-xs">
                          <q-item v-for="(value, name) in config.options.properties.userProperties" :key="`${name}: ${value}`" style="min-height: 17px;">
                            <q-icon class="q-mr-sm cursor-pointer" size='1rem' @click="removePublishUserProperty(name)" name="mdi-close-circle"/>
    ...
    addPublishUserProperty () {
      ...
      if (userProperties[this.publishUserProperty.name]) {
        if (Array.isArray(userProperties[this.publishUserProperty.name])) {
          ...
          userProperties[this.publishUserProperty.name] = [userProperties[this.publishUserProperty.name], this.publishUserProperty.value]
        }
      } else {
        userProperties[this.publishUserProperty.name] = this.publishUserProperty.value
      }
